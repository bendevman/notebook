...
<?php 
  ...
  if (isset($_POST['edit-btn'])) {
    ...
    $updateUser = "update users set login = '$login', password = '$passwordHash' where id = '$id'";
    if (!$errors) {
    ...
    $chekForUserSql = "select * from users where id = '$id'";
    $userResult = mysqli_query($db, $chekForUserSql);
...
<div class="user-info">
  <ul class="user-info__list">
    <li class="user-info__item"><?php echo 'Login: '. $login?></li>
  </ul>
</div>
<div class="edit-user">
  <h2 class="title"><?php echo 'Edit user';?></h2>
  <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST" class="register__form form">
    ...
    <input class="register__btn btn" name="edit-btn" type="submit" value="Edit">
